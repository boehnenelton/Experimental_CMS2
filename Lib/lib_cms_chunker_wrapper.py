...
import lib_mfdb_chunker_v5 as ChunkerV5
...
class CMS_Chunker_Wrapper:
    ...
    def backup_site(self, changelog: str = "Automated CMS Backup", tags: str = "backup,cms"):
        ...
        try:
            # 1. Perform Chunking
            # Note: ChunkerV5 creates .mfdb/ folder inside target_dir
            result = ChunkerV5.do_chunk(str(self.www_root), changelog=changelog, tags=tags)
            ...
            timestamp = ChunkerV5.get_timestamp()
            zip_filename = f"site_backup_{version}_{timestamp}.zip"
            zip_path = self.backup_dir / zip_filename
            
            export_result = ChunkerV5.do_export(str(manifest_path), version, str(zip_path))
            if export_result.get("ok"):
                # 3. Automatic Bump for next backup
                ChunkerV5.do_bump(str(self.www_root), part="patch")
    ...
    def restore_site(self, zip_filename: str):
        ...
        try:
            ...
            # 1. Import Zip
            import_result = ChunkerV5.do_import(str(temp_manifest), str(zip_path), overwrite=True)
            if not import_result.get("ok"):
                return {"success": False, "error": import_result.get("message")}
            
            # 2. Get version from imported manifest
            versions = ChunkerV5.list_versions(str(temp_manifest))
            if not versions:
            ...
            # 3. Unchunk
            unchunk_result = ChunkerV5.do_unchunk(str(temp_manifest), target_version)
