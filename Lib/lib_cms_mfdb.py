...
class MFDB_CMS_Manager:
    ...
    def get_pages_in_category(self, category_slug: str) -> List[Dict]:
        ...
        return [p for p in pages if p["category_fk"] == category_slug]

    def get_apps_in_category(self, category_slug: str) -> List[Dict]:
        apps = MFDBCore.mfdb_core_load_entity(self.content_manifest, "StandaloneApp")
        return [a for a in apps if a["category_fk"] == category_slug]
    ...
    def get_full_page_data(self, page_uuid: str) -> Optional[Dict]:
        ...
        content = next((c for c in contents if c["page_uuid_fk"] == page_uuid), {})
        return {**page, **content}
    ...
    def get_asset_by_hash(self, file_hash: str) -> Optional[Dict]:
        ...
        return next((a for a in assets if a["file_hash"] == file_hash), None)
    ...
    def create_app(self, name: str, description: str, category: str, featured_img: str, entry_file: str):
        app_uuid = str(uuid.uuid4())
        slug = name.lower().replace(" ", "-")
        created_at = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        MFDBCore.mfdb_core_add_entity_record(self.content_manifest, "StandaloneApp", [app_uuid, name, slug, description, category, featured_img, entry_file, created_at])
