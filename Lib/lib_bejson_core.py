...
class BEJSONCoreError(Exception): pass
...
def bejson_core_add_record(doc, record):
    doc['Values'].append(record)
    return True

def bejson_core_remove_record(doc, index):
    if index < len(doc['Values']):
        doc['Values'].pop(index)
        return True
    return False

def bejson_core_update_field(doc, row_idx, field_idx, value):
    doc['Values'][row_idx][field_idx] = value
    return True

def bejson_core_get_record_count(doc):
    return len(doc.get('Values', []))

def bejson_core_filter_rows(doc, field_idx, value):
    return [r for r in doc['Values'] if r[field_idx] == value]

def bejson_core_sort_by_field(doc, field_idx, reverse=False):
    doc['Values'].sort(key=lambda x: x[field_idx], reverse=reverse)

def bejson_core_load_string(content: str) -> Optional[dict]:
    """Loads a BEJSON document from a string."""
    try:
        return json.loads(content)
    except Exception as e:
        logging.error(f"[BEJSON_CORE] Failed to load BEJSON from string: {e}")
        return None
